...
bbox <- c(left=103.5, right=110.2, bottom=51.4, top=56)
...
ggmap(BaikalMap) + #+ xlab("Longitude") + ylab("Latitude") +
  geom_point(data = sampling_points, aes(x = lon, y = lat), col = sampling_points$col, alpha = 0.5, size = 2) +
#  geom_rect(xmin = 103.75, xmax = 105.75, ymin = 51.5, ymax = 52, col = "grey50", fill = 'NA') + 
  theme_classic(base_size = 22) + theme(panel.border = element_rect(colour = "black", fill=NA, linewidth=1))

ggsave("map.svg", device=svg, width = 5.72*2, height = 4.29*2)
ggsave("map.png", device=png, width = 5.72*2, height = 4.29*2)
...
#ggsave("map_inset.png", width = 5.72, height = 4.29)
